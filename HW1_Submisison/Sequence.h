...
#define Sequence_h
#include <iostream>
#include <string> //the program still builds on xcode without this included

//using ItemType = std::string;
using ItemType = unsigned long;
const int DEFAULT_MAX_ITEMS = 160;//global value for max number of elements in a sequence

class Sequence
{
    
  private:
    ItemType sequence[DEFAULT_MAX_ITEMS];//an array sequence of whatever ItemType is defined to be
    int m_size;//a counter variable that keeps track of how many elements are in the array
  ...
  public:
    ...
    void dump();
      //Prints out all values in the sequence
};
